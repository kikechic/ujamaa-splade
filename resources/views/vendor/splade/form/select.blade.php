<SpladeSelect class="splade" {{ $attributes->only(['v-if', 'v-show', 'class']) }} :choices="@js($choicesOptions())"
    ...
    <template #default="{!! $scope !!}">
        <label class="block" v-bind:class="{ 'pointer-events-none': select.loading }">
            ...
            <div class="relative h-full">
                <div v-bind:class="{ 'opacity-50': select.loading, 'h-full': true }">
                    <select
                        {{ $attributes->except(['v-if', 'v-show', 'class'])->class([
                                'block w-full h-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 disabled:opacity-50 text-xs',
                            ])->merge([
                                'multiple' => $multiple,
                                'name' => $name,
                ...
                <div class="absolute inset-0 w-full h-full" v-if="select.loading">
                    <div class="flex flex-row items-center justify-center w-full h-full">
                        <svg class="w-5 h-5 mr-3 text-indigo-500 animate-spin" xmlns="http://www.w3.org/2000/svg"
