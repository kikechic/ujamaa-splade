<div class="flex flex-row px-4 mb-3 -mr-2 sm:justify-end sm:px-0 sm:-mr-3">
    ...
    @if ($table->searchInputs('global'))
        <div class="flex-grow order-3 w-full mr-2 sm:mr-3">
    ...
    <button v-show="@js($canResetTable()) || table.columnsAreToggled || table.hasForcedVisibleSearchInputs" type="button"
        class="order-6 sm:order-4 ml-auto mr-2 sm:mr-3 bg-white border rounded-md shadow-sm px-2.5 sm:px-4 py-2 inline-flex justify-center text-xs font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 border-gray-300"
        ...
        <span class="hidden ml-2 sm:inline">{{ __('Reset') }}</span>
    </button>

    @if ($table->hasToggleableSearchInputs())
        <div class="order-4 mr-2 sm:order-5 sm:mr-3">
            @include('splade::table.add-search-row')
        </div>
    @endif

    @if ($table->hasToggleableColumns())
        <div class="order-5 mr-2 sm:order-6 sm:mr-3">
