<nav role="navigation" aria-label="{{ __('Pagination Navigation') }}"
    class="flex items-center justify-between px-4 py-3 sm:px-0">
    <div class="flex justify-between flex-1 md:hidden">
        ...
        @if ($paginator->hasMorePages())
            <a @click.exact.prevent="table.navigate(@js($paginationUrl = $paginator->nextPageUrl()), true)" dusk="pagination-simple-next"
                href="{{ $paginationUrl }}"
                class="relative inline-flex items-center px-4 py-2 ml-3 text-xs font-medium leading-5 text-gray-700 transition duration-150 ease-in-out bg-white border border-gray-300 rounded-md sm:text-xs hover:text-gray-500 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-700">
                {!! __('pagination.next') !!}
            </a>
        @else
            <span
    ...
    <div class="hidden md:flex-1 md:flex md:items-center md:justify-between">
        <div class="flex items-center">
            ...
            <div class="hidden lg:block @if ($hasPerPageOptions ?? true) ml-3 @endif">
                <p class="text-xs leading-5 text-gray-700 sm:text-xs">
            ...
            <span class="relative z-0 inline-flex rounded-md shadow-sm">
                ...
                @if ($paginator->onFirstPage())
                    <span aria-disabled="true" aria-label="{{ __('pagination.previous') }}">
                        <span
                            class="relative inline-flex items-center px-2 py-2 text-xs font-medium leading-5 text-gray-500 bg-white border border-gray-300 cursor-default sm:text-xs rounded-l-md"
                            aria-hidden="true">
                ...
                @else
                    <a @click.exact.prevent="table.navigate(@js($paginationUrl = $paginator->previousPageUrl()), true)" dusk="pagination-previous"
                        ...
                        class="relative inline-flex items-center px-2 py-2 text-xs font-medium leading-5 text-gray-500 transition duration-150 ease-in-out bg-white border border-gray-300 sm:text-xs rounded-l-md hover:text-gray-400 focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-500"
                ...
                @foreach ($elements as $element)
                    ...
                    @if (is_string($element))
                        <span aria-disabled="true">
                            <span
                                class="relative inline-flex items-center px-4 py-2 -ml-px text-xs font-medium leading-5 text-gray-700 bg-white border border-gray-300 cursor-default sm:text-xs">{{ $element }}</span>
                        </span>
                    @endif
                ...
                @if ($paginator->hasMorePages())
                    <a @click.exact.prevent="table.navigate(@js($paginationUrl = $paginator->nextPageUrl()), true)" dusk="pagination-next"
                        ...
                        class="relative inline-flex items-center px-2 py-2 -ml-px text-xs font-medium leading-5 text-gray-500 transition duration-150 ease-in-out bg-white border border-gray-300 sm:text-xs rounded-r-md hover:text-gray-400 focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-500"
                ...
                @else
                    <span aria-disabled="true" aria-label="{{ __('pagination.next') }}">
                        <span
                            class="relative inline-flex items-center px-2 py-2 -ml-px text-xs font-medium leading-5 text-gray-500 bg-white border border-gray-300 cursor-default sm:text-xs rounded-r-md"
                            aria-hidden="true">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd"
                                    d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z"
