<x-splade-component is="button-with-dropdown" dusk="bulk-actions-exports-dropdown" v-bind:close-on-click="true">
    ...
    <div class="bg-white rounded-md shadow-lg min-w-max ring-1 ring-black ring-opacity-5">
        <div class="flex flex-col">
            <h3 v-if="table.hasSelectedItems" class="px-4 py-2 text-xs tracking-wide uppercase bg-gray-100 border-b">
            ...
            @foreach ($table->getBulkActions() as $bulkAction)
                <button v-if="table.hasSelectedItems"
                    class="w-full px-4 py-2 text-xs font-normal text-left text-gray-700 hover:bg-gray-50 hover:text-gray-900"
            ...
            @if ($table->hasExports() && $table->hasBulkActions())
                <div v-if="table.hasSelectedItems" class="w-full border-t"></div>
            @endif

            @if ($table->hasExports())
                <h3 class="px-4 py-2 text-xs tracking-wide uppercase bg-gray-100 border-b">
                    {{ __('Export results') }}
                </h3>
            @endif
