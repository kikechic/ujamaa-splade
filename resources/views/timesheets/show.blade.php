			...
			<x-dialog-modal>
				...
				<x-slot:content>
					<x-splade-data :default="['status' => $timesheet->status->value]">
						<x-splade-form id="send-approval" preserve-scroll method="post" confirm confirm-text="Send approval request?" action="{{ route('approvalRequests.store', ['timesheet'=> $timesheet]) }}" />
						<!-- <x-splade-form id="send-approval" preserve-scroll method="post" confirm confirm-text="Send approval request?" action="{{ route('approvalRequests.store', ['timesheet'=> $timesheet]) }}" @success="if(state.shared.approval_request_sent) { $splade.emit('timesheet-status-updated-' + {{ $timesheet->id }}); data.status = `{{ \App\Enums\TimesheetStatusEnum::pending() }}`; }" /> -->
