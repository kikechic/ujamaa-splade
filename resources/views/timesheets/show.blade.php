			...
			<x-dialog-modal>
				...
				<x-slot:content>
					<x-splade-data :default="['status' => $timesheet->status->value]">
						<x-splade-form id="send-approval" stay preserve-scroll method="post" confirm confirm-text="Send approval request?" action="{{ route('approvalRequests.store', [
						...
						<x-splade-form id="cancel-approval" preserve-scroll confirm confirm-text="Cancel approval request?" action="{{ route('approvalRequests.reject', [
							    'documentable_id' => $timesheet->id,
							    'documentable_code' => 'timesheet',
							]) }}" method="PUT" @success="$splade.emit('timesheet-status-updated-' + {{ $timesheet->id }}); data.status = `{{ \App\Enums\TimesheetStatusEnum::open() }}`">
						</x-splade-form>

						<x-splade-form id="approve" preserve-scroll confirm confirm-text="Approve request?" action="{{ route('approvalRequests.approve', [
							    'documentable_id' => $timesheet->id,
							    'documentable_code' => 'timesheet',
							]) }}" method="PUT" @success="$splade.emit('timesheet-status-updated-' + {{ $timesheet->id }}); data.status = `{{ \App\Enums\TimesheetStatusEnum::approved() }}`">
						</x-splade-form>
